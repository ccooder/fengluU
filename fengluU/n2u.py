...
CN_UNIT = ['万', '亿', '兆', '京', '垓', '秭', '穰', '沟', '涧', '正', '载', '极', '归', '无量']
        ...
        if num_bit == 0:
            if CN_UNIT.count(unit) == 1:
                ...
                if upper_num.endswith('零'):
                    ...
                    while not ['十', '百', '千'].count(last_unit) and not CN_UNIT.count(last_unit):
                        stop -= 1
                        last_unit = upper_num[-1:stop:-1][::-1]
                    if CN_UNIT.count(last_unit):
                        if CN_UNIT.index(unit) - CN_UNIT.index(last_unit) < 1:
                            upper_num += '零'
                            continue
                upper_num += unit
